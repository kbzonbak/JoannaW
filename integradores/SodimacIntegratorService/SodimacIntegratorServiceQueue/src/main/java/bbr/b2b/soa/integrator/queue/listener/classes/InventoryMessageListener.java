...
public class InventoryMessageListener {
	...
	@JmsListener(destination = QueueDefinitions.Q_INV, containerFactory = "defaultJmsListenerContainerFactory")
	@Transactional
	public void receiveMessage(@Payload String content,
		...
		try {
			...
			if (correlationId.equals("none") || vendorCode.equals("none")) {
				...
				LOGGER.info("Mensaje recibido - Cola: " + QueueDefinitions.Q_INV + " - Id: " + correlationId);
				
				processor.processInvMessage(content, vendorCode);
			}  
		} catch (Exception ex) {
          	ex.printStackTrace();
		}	
