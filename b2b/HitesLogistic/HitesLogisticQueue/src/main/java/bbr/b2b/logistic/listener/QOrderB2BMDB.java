...
@MessageDriven(name = "listeners/QOrderB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/ql_ordenes"),
...
public class QOrderB2BMDB implements MessageListener {
	...
	private static Logger logger = Logger.getLogger(QOrderB2BMDB.class);	
	...
	private String businessLogic(String headervalues, String message) {
		...
		try{
			...
			if (sendmailmq) {
				String subject = LogisticConstants.getBUSINESSAREA() + LogisticConstants.getCOUNTRYCODE() + "Logistica: Mensaje de OC Canónico";
			...
			orderhandler.handleMessage(headervalues, message, mdbContext);
	...
	public void onMessage(Message message) {
		...
		try {
			if (message instanceof TextMessage) {
				...
				String subject = LogisticConstants.getBUSINESSAREA() + LogisticConstants.getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de OC Can�nico";
				try {
					Mailer mailer = Mailer.getInstance();
