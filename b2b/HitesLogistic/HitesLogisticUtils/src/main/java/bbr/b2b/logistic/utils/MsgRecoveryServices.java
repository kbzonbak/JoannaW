...
public class MsgRecoveryServices {
	...
	private void getParams(String ruta) throws OperationFailedException {
		try {
			...
			dir_out = config.getString("bbr.msgrec.dir.out");
			dir_err = config.getString("bbr.msgrec.dir.err");
			dir_gen = config.getString("bbr.msgrec.dir.gen");
			dir_lostmsges = config.getString("bbr.msgrec.dir.lostmsges");
	...
	private void mailMe(String msg, String msgetype, String path, Exception exception) {
		...
		String subject = "";
		try {
			subject = LogisticConstants.getBUSINESSAREA() + LogisticConstants.getCOUNTRYCODE() +"Logistica: Mensaje no se quiere procesar. MsgeType :" + msgetype;
			...
			if (file.isFile()) {
				...
				for (int i = 0; i < lenLista; i++) {
					...
					try {
						...
						destFileStr = dir_out + finalStr;
						File destFile = new File(destFileStr);
						logger.debug("ARCHIVO DESTINO --> " + destFileStr);
						 
						Files.move(fileToMove.toPath(), destFile.toPath(),  StandardCopyOption.REPLACE_EXISTING);
												
					} catch (IOException e) {
						logger.info("Error moviendo archivo en recovery");
						e.printStackTrace();
					}
				}
			}
