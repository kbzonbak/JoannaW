package bbr.b2b.logistic.vev.rest.classes;
...
import bbr.b2b.logistic.vev.rest.interfaces.IDODeliveryAPIRESTManager;
...
public class DODeliveryAPIRESTManager implements IDODeliveryAPIRESTManager {
	...
	private DeliveryReportManagerLocal deliveryReportManagerLocal;
	...
	private void getHomes() throws OperationFailedException {
		try {
			...
			objectManagerReference = (new InitialContext()).lookup("java:global/HitesLogisticEAR/HitesLogisticManager/managers/DeliveryReportManager!bbr.b2b.logistic.deliveries.managers.interfaces.DeliveryReportManagerLocal");
			deliveryReportManagerLocal = (DeliveryReportManagerLocal) PortableRemoteObject.narrow(objectManagerReference, DeliveryReportManagerLocal.class);
	...
	public Response doAddDODeliveryOfDirectOrdersWS(@ApiParam(name="initParams", value="Datos de la orden", required=true) AddDODeliveryOfDirectOrdersWSInitParamDTO initParams) {
		...
		try {
			...
			if (initParams.getOrdenes() != null && initParams.getOrdenes().length > 0 &&
				...
				AddDODeliveryOfDirectOrdersWSResultDTO doDeliveryAddArrayResultDTO = deliveryReportManagerLocal.doAddDODeliveryOfDirectOrdersWS(initParams);
			...
			AddDODeliveryOfDirectOrdersWSResultDTO baseResult = LogisticStatusCodeUtils.getInstance().setStatusCode(new AddDODeliveryOfDirectOrdersWSResultDTO(), "L1");
...
//				}
				...
				// LLAMA A SERVICIO MANAGER LOCAL
				UpdateDODeliveryWSResultDTO doDeliveryStateResultDTO = deliveryReportManagerLocal.doUpdateDODeliveryWS(initParams);
			...
			UpdateDODeliveryWSResultDTO baseResult = LogisticStatusCodeUtils.getInstance().setStatusCode(new UpdateDODeliveryWSResultDTO(), "L1");
