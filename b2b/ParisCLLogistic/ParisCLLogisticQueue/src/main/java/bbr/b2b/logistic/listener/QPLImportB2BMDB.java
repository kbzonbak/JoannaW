...
import bbr.b2b.logistic.queuehandler.PLImportHandlerLocal;
...
@MessageDriven(name = "listeners/QPLImportB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_plimp"),
...
public class QPLImportB2BMDB implements MessageListener{
	...
	@EJB
	PLImportHandlerLocal plimporthandler;

	private String businessLogic(String message) {
		try {
			// ---------------------- MAIL -------------------------
			if (RegionalLogisticConstants.getInstance().getSENDMAILTESTCSUD()) {
				String subject = "Carga en B2B Test: Mensaje de Packing List Internacional";
			...
			plimporthandler.handleMessage(message, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de Packing List Internacional";
				try {
