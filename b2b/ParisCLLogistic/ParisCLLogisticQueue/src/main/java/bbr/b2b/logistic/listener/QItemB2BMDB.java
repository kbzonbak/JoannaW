...
import bbr.b2b.logistic.queuehandler.ItemHandlerLocal;
...
@MessageDriven(name = "listeners/QItemB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_productos"),
...
public class QItemB2BMDB implements MessageListener {
	...
	private String businessLogic(String message) {
		try {
			...
			itemhandler.handleMessage(message, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de productos";
				try {
