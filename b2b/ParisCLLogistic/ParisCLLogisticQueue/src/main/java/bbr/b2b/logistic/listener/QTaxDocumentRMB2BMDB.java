...
import bbr.b2b.logistic.queuehandler.TaxDocumentRMHandlerLocal;
...
@MessageDriven(name = "listeners/QTaxDocumentRMB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_rm"),
...
public class QTaxDocumentRMB2BMDB implements MessageListener {
	...
	private String businessLogic(String message) {
		try {
			...
			taxdocumentrmhandler.handleMessage(message, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logística: Fallo al parsear mensaje de RM";
				try {
					...
					logger.error("No se envió el mail del fracaso de carga de mensaje");
				}
			}
