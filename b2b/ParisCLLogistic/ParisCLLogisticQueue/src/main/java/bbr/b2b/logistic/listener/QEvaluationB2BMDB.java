...
import bbr.b2b.logistic.queuehandler.EvaluationHandlerLocal;
...
@MessageDriven(name = "listeners/QEvaluationB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_evaluacion"),
...
public class QEvaluationB2BMDB implements MessageListener{
	...
	@EJB
	EvaluationHandlerLocal evaluationhandler;

	private String businessLogic(String message) {
		try {
			// ---------------------- MAIL -------------------------
			if (RegionalLogisticConstants.getInstance().getSENDMAILTESTCSUD()) {
				String subject = "Carga en B2B Test: Mensaje de Evaluación de Recepción";
			...
			evaluationhandler.handleMessage(message, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de Evaluación de Recepción";
				try {
