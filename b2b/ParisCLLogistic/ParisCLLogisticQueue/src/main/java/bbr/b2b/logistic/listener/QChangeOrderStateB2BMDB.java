...
import bbr.b2b.logistic.queuehandler.ChangeOrderStateHandlerLocal;
...
@MessageDriven(name = "listeners/QChangeOrderStateB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_cambioestado"),
...
public class QChangeOrderStateB2BMDB implements MessageListener {
	...
	@EJB
	ChangeOrderStateHandlerLocal changestatehandler;

	private String businessLogic(String message, String correlationid) {
		try {
			// ---------------------- MAIL -------------------------
			if (RegionalLogisticConstants.getInstance().getSENDMAILTESTCSUD()) {
				String subject = "Carga en B2B Test: Mensaje de Cambio de Estado";
			...
			changestatehandler.handleMessage(message, correlationid, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				businessLogic(req, correlationid);
			} else {
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de cambio de estado";
				try {
