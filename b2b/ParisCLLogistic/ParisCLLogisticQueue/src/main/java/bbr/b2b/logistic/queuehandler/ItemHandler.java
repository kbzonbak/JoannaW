...
import bbr.b2b.logistic.msgregionalb2b.XmlToItemLocal;
...
@Stateless(name = "handlers/ItemHandler")
@TransactionManagement(TransactionManagementType.BEAN)
public class ItemHandler implements ItemHandlerLocal {
	...
	private static JAXBContext getJC() throws JAXBException {
		if (jc == null)
			jc = JAXBContext.newInstance("bbr.b2b.regional.logistic.xml.qitem");
		return jc;
	}	
	...
	private void doSendResponse(String number, String messagetype, Integer state, String exception, String date, String content)  {
		...
		try {
			...
			// planifica el envio
			schedulermanager.doAddMessageQueue("jboss/qcf_pariscl", "jboss/wsmq/q_ackb2b", "ackProducto", numberStr, result);
...
//			}			
	...
	public void handleMessage(String content, javax.ejb.MessageDrivenContext ctx) throws OperationFailedException, NotFoundException {
		...
		try {	
			...
			QPRODUCTO itemParser = (QPRODUCTO) u.unmarshal(source);
		
			// Obtiene el numero de orden
			try {
				sku = itemParser.getCodProducto().trim();				
			} catch (Exception e) {
				throw new LoadDataException("QPRODUCTO: No se puede obtener el sku de producto");
			}
			
			logger.info("Inicio de carga de Producto: " + sku);	

			// RESPALDA MENSAJE			
			doBackUpMessage(content, sku, "PR");
			...
			sku = xmltooitem.processMessage(itemParser);

			// Commit de la transacion
			usertransaction.commit();
			logger.info("Item: " + sku + " cargado");
			
			// Se reporta el estado de la transacción
			doSendResponse(sku, "PR", 0, "El producto se cargo correctamente", now, content);
			...
			// Respalda mensaje
			doBackUpMessage(content, "XXX", "PR");
			
			// Envia mensaje de error
			doSendResponse("-1", "PR", 1, "La estructura del mensaje no es v�lida", now, content);
			...
			doSendResponse(sku, "PR", 1, e.getMessage(), now, content);
			...
			String numberStr = "";
			try {
				String[] contentSp = content.split("<cod_producto>");
				numberStr = contentSp[1].substring(0, contentSp[1].indexOf("</cod_producto>"));
			} catch (Exception e) {
				numberStr = "0";
			}

			messagetype = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "_PRODUCTO_" + numberStr;
			try {
