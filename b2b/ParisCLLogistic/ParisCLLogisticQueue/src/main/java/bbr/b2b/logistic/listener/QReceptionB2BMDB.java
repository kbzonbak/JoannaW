...
@MessageDriven(name = "listeners/QReceptionB2BMDB", activationConfig = {
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "java:jboss/wsmq/q_recepcion"),
	...
	private String businessLogic(String message) {
		try {
			...
			if (RegionalLogisticConstants.getInstance().getSENDMAILTESTCSUD()) {
				String subject = "Carga en B2B Test: Mensaje de Recepción";
			...
			receptionhandler.handleMessage(message, mdbContext);
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof TextMessage) {
				...
				String subject = RegionalLogisticConstants.getInstance().getBUSINESSAREA() + RegionalLogisticConstants.getInstance().getCOUNTRYCODE() + "Logistica: Fallo al parsear mensaje de recepción";
				try {
