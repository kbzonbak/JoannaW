...
import bbr.b2b.portal.components.filter_section.generic.ProviderFilterSectionCustom;
import bbr.b2b.portal.components.filter_section.stockpool.AvailableStockCriterionFilterSection;
...
public class AvailableStockReportFilter extends BbrFilterContainer implements Button.ClickListener
{
	...
	private ProviderFilterSectionCustom				providerFilterSection	= null;
	private AvailableStockCriterionFilterSection	criterionFilterSection	= null;
	private String									codeState				= null;
	// =====================================================================================================================================
	// ENDING SECTION ----> PROPERTIES
	// =====================================================================================================================================

	public AvailableStockReportFilter(BbrModule parentModule, String codeState)
	{
		super(parentModule);
	...
	public void buttonClick(ClickEvent event)
	{
		...
		if ((errorMsg == null) || (errorMsg.length() == 0))
		{
			...
			AvailableStockInitParamDTO initParam = this.getInitParam();
		...
		else
		{
			...
			AvailableStockReportFilter.this.showWarningMessage(I18NManager.getI18NString(BbrUtilsResources.RES_GENERIC, "windows_title_warning"), errorMsg);
		}
	}
	...
	{
		// Seccion Proveedor
		providerFilterSection = new ProviderFilterSectionCustom(this.getBbrUIParent());
		...
		// Sección Criterio
		criterionFilterSection = new AvailableStockCriterionFilterSection(this.getBbrUIParent(), codeState);
	...
	private BbrFilterEvent getBbrFilterEventObject()
	{
		...
		SPLCriterionDTO selected = this.criterionFilterSection.getSectionResult();
		...
		if (selected != null)
		{
			filterBy = PucharseOrderSearchFilterUtils.getFilterCaption(PucharseOrderSearchFilterUtils.RES_FILTERS, "filter_by", true) + selected.getTypeSearch().getDescription();
			valueBy = PucharseOrderSearchFilterUtils.getFilterCaption(PucharseOrderSearchFilterUtils.RES_FILTERS, "value", true) +  selected.getSku();
		}
		
		result.setSecondaryCaption(filterBy + " - " + valueBy);
	...
	private AvailableStockInitParamDTO getInitParam()
	{
		AvailableStockInitParamDTO result = new AvailableStockInitParamDTO();
		boolean active = this.providerFilterSection.getActiveCheckBox();
		String rut = this.providerFilterSection.getSectionResult().getRut();

		SPLCriterionDTO selected = this.criterionFilterSection.getSectionResult();

		SPLCriterionDTO test = new SPLCriterionDTO(null, rut, null);
		test.setState(selected.getState());
		test.setSku(selected.getSku());

		result.setVendorcode(rut);
		result.setActive(active);

		result.setFiltertype(selected.getTypeSearch().getId());
		switch (selected.getTypeSearch().getId())
		{

			case 1:
				result.setSku(selected.getSku());
				break;
			case 2:
				result.setStatetype(selected.getState().getId());
				break;
		}
