...
public class FindSite extends BbrWindow
{
	// ****************************************************************************************
	...
	private BbrAdvancedGrid<SiteDTO>		dgd_Site			= null;
	private Button							btn_Select			= null;
	// Variables
	private List<SiteDTO>					listOfSites			= null;
	...
	// ****************************************************************************************
	public FindSite(BbrUI parent, List<SiteDTO> listOfSites)
	{
		super(parent);
	...
	{
		this.dgd_Site.addColumn(SiteDTO::getRetailname).setCaption(I18NManager.getI18NString(BbrUtilsResources.RES_SEARCH, "description"));
	}
	...
	private void select_clickHandler(ClickEvent e)
	{
		...
		{
			SiteDTO site = (SiteDTO) dgd_Site.getSelectedItems().iterator().next();
	...
	private void search_clickHandler(ClickEvent e)
	{
		...
		if (errorMessage == null || errorMessage.length() <= 0)
		{
			try
			{
				...
				{
					List<SiteDTO> items = listOfSites.stream()
							.sorted(Comparator.comparing(SiteDTO::getRetailname))
							.filter(str -> str.getRetailname().contains(value.toUpperCase().trim()))
				...
				{
					this.dgd_Site.setItems(new ArrayList<SiteDTO>());
