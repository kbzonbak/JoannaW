 ...
 * @author DSU 2017-04
 */
public class RemoveCompanies extends BbrWindow implements EntityEditor<UserDTO>
{
	// ****************************************************************************************
	...
	private BbrAdvancedGrid<CompanyDataDTO>	dgd_RemovedCompanies;
	private BbrAdvancedGrid<CompanyDataDTO>	dgd_FoundCompanies;

	private ArrayList<CompanyDataDTO>		dsRemovedCompanies		= new ArrayList<>();
	private ArrayList<CompanyDataDTO>		dsAvailableCompanies	= null;

	private HashMap<Long, CompanyDataDTO>	toRemoveCompanies		= new HashMap<>();
	...
	// ****************************************************************************************
	public RemoveCompanies(BbrUI parent, UserDTO selectedUser)
	{
		super(parent);
	...
	public void initializeView()
	{
		...
		dgd_RemovedCompanies = new BbrAdvancedGrid<>(this.getUser().getLocale());
		dgd_RemovedCompanies.addColumn(CompanyDataDTO::getRut).setCaption(I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_MANAGEMENT, "code"));
		dgd_RemovedCompanies.addColumn(CompanyDataDTO::getName).setCaption(I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_MANAGEMENT, "description"));
		dgd_RemovedCompanies.setSortable(false);
		dgd_RemovedCompanies.setSelectionMode(SelectionMode.MULTI);
		dgd_RemovedCompanies.addStyleName("report-grid");
		dgd_RemovedCompanies.addStyleName("bbr-multi-grid");
		dgd_RemovedCompanies.setSizeFull();
		...
		assignedPanel.addComponents(pnlAssignedOperation, dgd_RemovedCompanies, pnlCounter);
		assignedPanel.setMargin(false);
		assignedPanel.setExpandRatio(dgd_RemovedCompanies, 1F);
		...
		this.setCaption(I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_MANAGEMENT, "win_title_remove_company"));
	...
	{
		dsRemovedCompanies = new ArrayList<>();

		toRemoveCompanies = new HashMap<>();
	...
	private void remove_clickHandler(ClickEvent e)
	{
		...
		dgd_RemovedCompanies.getSelectedItems().forEach(company ->
		{

			Predicate<CompanyDataDTO> companyPredicate = p -> p.getId().equals(company.getId());

			CompanyDataDTO cmp = toRemoveCompanies.remove(company.getId());

			dgd_FoundCompanies.removeItemSelection(cmp.getId());

			dsRemovedCompanies.removeIf(companyPredicate);
		});
	...
	{
		if (dsRemovedCompanies != null && dsRemovedCompanies.size() > 0)
		{

			Long[] ids = new Long[dsRemovedCompanies.size()];
			for (int i = 0; i < dsRemovedCompanies.size(); i++)
			{
				ids[i] = new Long(dsRemovedCompanies.get(i).getId());
			}
	...
	private void addAllCompanies_clickHandler(ClickEvent e)
	{
		if (dsAvailableCompanies != null && dsAvailableCompanies.size() > 0)
		{
			...
			{
				if (toRemoveCompanies.get(company.getId()) == null)
				{
					toRemoveCompanies.put(company.getId(), company);
					dsRemovedCompanies.add(company);
				}
			});
	...
	private void doUpdateCompanies(Long[] ids)
	{
		...
		try
		{
			...
			{
				BaseResultDTO addResult = EJBFactory.getUserEJBFactory().getUserReportManagerLocal().deleteUserCompanyRelations(this.item.getId(), ids, this.getUser().getId());
				if (addResult != null)
				{
	...
	private void updateCompanies(int pageNumber, boolean resetPageInfo, boolean isPaginated)
	{
		...
		if (errorMessage == null || errorMessage.length() <= 0)
		{
			...
			try
			{
				...
				{
					companiesResult = EJBFactory.getUserEJBFactory().getFunctionalityManagerLocal().findCompaniesLikeNameAssigned(this.getItem().getId(), value, pageNumber, this.MAX_ROWS_NUMBER, resetPageInfo, isPaginated, null);
				}
				else if (criterion.getId() == 1)
				{
					companiesResult = EJBFactory.getUserEJBFactory().getFunctionalityManagerLocal().findCompaniesLikeCodeAssigned(this.getItem().getId(), value, pageNumber, this.MAX_ROWS_NUMBER, resetPageInfo, isPaginated, null);
				}
				else if (criterion.getId() == 2)
				{
					companiesResult = EJBFactory.getUserEJBFactory().getFunctionalityManagerLocal().findAllCompaniesAssigned(this.getItem().getId(), pageNumber, this.MAX_ROWS_NUMBER, resetPageInfo, isPaginated, null);
				}
	...
	{
		ListDataProvider<CompanyDataDTO> dataprovider = new ListDataProvider<>(dsRemovedCompanies);
		dgd_RemovedCompanies.setDataProvider(dataprovider);
	}
	...
	private void updateToRemoveSelection(Set<CompanyDataDTO> selection)
	{
		...
		if (resultOperation != null && resultOperation.getState() != EnumDifferenceState.NONE)
		{
			...
			else if (resultOperation.getState() == EnumDifferenceState.NEGATIVE)
			{
				...
				resultOperation.getSetResult().forEach(company ->
				{
					toRemoveCompanies.remove(company.getId());
					if (dsRemovedCompanies != null)
					{
						Predicate<CompanyDataDTO> companyPredicate = p -> p.getId().equals(company.getId());
	...
	private void updateSelectionLabel()
	{
		if (dsRemovedCompanies != null && dsRemovedCompanies.size() > 0)
		{
			...
			{
				lblCounter.setValue(I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_MANAGEMENT, "more_companies_selecteds", String.valueOf(dsRemovedCompanies.size())));
			}
		}
		else
		{
