...
public class TemplateGenAttributesEditor extends BbrWindow implements BbrWorkExecutor
{
	...
	private static final String						FOUND_ATTRIB_VENDOR_NAME		= "endorname";
	...
	private BbrAdvancedGrid<DefinableAttributeDataDTO>	datGrid_SelectedAttribs		= null;
	private Label									lblCounterAssigned			= new Label();
	private Label									lblCounterAvailable			= new Label();

	private ArrayList<DefinableAttributeDataDTO>			dsAssignedAttribs			= new ArrayList<>();
	private ArrayList<AttributeDTO>					dsAvailableAttribs			= null;
	private ArrayList<DefinableAttributeDataDTO>			selectedDefAttribList		= new ArrayList<DefinableAttributeDataDTO>();
	...
	public TemplateGenAttributesEditor(BbrUI parent, ArrayList<DefinableAttributeDataDTO> currentlySelectedAttributes)
	{
		super(parent);
	...
	public void initializeView()
	{
		...
		pnl_FoundAttribsSpace.addComponents(new BbrVSeparator("30px"), lbl_GenericAttribFound, templateSearchAttribSection, this.datGrid_FoundAttribs, pnlCounterAvailable);
		...
		pnl_RemoveBtnSpace.addComponent(btn_RemoveSelectedAttributes);
		pnl_RemoveBtnSpace.setComponentAlignment(btn_RemoveSelectedAttributes, Alignment.MIDDLE_LEFT);
		...
		this.datGrid_SelectedAttribs.addSelectionListener((SelectionListener<DefinableAttributeDataDTO> & Serializable) e -> {
			this.updateButtons();
		});
		...
		pnl_SelectedAttribsSpace.addComponents(pnl_RemoveBtnSpace, lbl_GenericAttribSelected, this.datGrid_SelectedAttribs, pnlCounterAssigned);
		...
		this.btn_Save.addClickListener((ClickListener & Serializable) e -> btnUpdateAttribs_clickHandler(e));
		...
		this.setWidth("90%");
		this.setHeight("90%");
		this.setCaption(I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "title_attributes_finder"));
	...
	{
		TemplateGenAttributesEditor bbrSender = (TemplateGenAttributesEditor) sender;
		if (result != null)
		{
	...
	private void attribSearch_clickHandler(ClickEvent e)
	{
		...
		if (errorMessage == null || errorMessage.length() <= 0)
		{
			...
			this.searchResultsWork.addFunction(new Function<Object, AttributeArrayResultDTO>()
			{
				...
				{
					return executeSearchAttributesService(TemplateGenAttributesEditor.this.getBbrUIParent());
				}
			});
	...
	private void btn_RemoveAttribute_clickHandler(ClickEvent event)
	{
		this.datGrid_SelectedAttribs.getSelectedItems().forEach(attrib -> {
			AttributeDTO pdt = this.toAssignAttribs.remove(attrib.getAttributeid());

			if (pdt != null)
				this.datGrid_FoundAttribs.deselect(pdt);

			this.dsAssignedAttribs.removeIf(a -> a.getAttributeid().equals(attrib.getAttributeid()));
		});
	...
	private void initializeSelectedAttribGridColumns()
	{
		...
		this.datGrid_SelectedAttribs.addCustomColumn(DefinableAttributeDataDTO::getAttributeinternalname,
		I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "b2b_Name"), true);
		
		this.datGrid_SelectedAttribs.addCustomColumn(DefinableAttributeDataDTO::getAttributevendorname,
		I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "col_vendor_name"), true);
		
		this.datGrid_SelectedAttribs.addCustomColumn(DefinableAttributeDataDTO::getAtributedatatypename,
		I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "col_attribute_type_name"), true);
		
		this.datGrid_SelectedAttribs.addCustomComponentColumn(d -> this.getMandatoryCheckBoxField(d), I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "col_require"), true)
		.setStyleGenerator(p -> BbrAlignment.CENTER.getValue())
		.setWidth(120D);
		
		this.datGrid_SelectedAttribs.addCustomComponentColumn(d -> this.getBulletCheckBoxField(d), I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "col_bullet"), true)
		.setStyleGenerator(p -> BbrAlignment.CENTER.getValue())
		.setWidth(120D)
		.setSortable(false);
	}
	...
	private void updateToAssignSelection(Set<AttributeDTO> addedSelection, Set<AttributeDTO> removedSelection, boolean isUserOriginated)
	{
		if (addedSelection != null && addedSelection.size() > 0)
		{
			addedSelection.forEach(attrib -> {
				...
				{
					DefinableAttributeDataDTO definableAttrib = new DefinableAttributeDataDTO();
					definableAttrib.setAttributeid(attrib.getId());
					definableAttrib.setAttributevendorname(attrib.getVendorname());
					definableAttrib.setAtributedatatypename(attrib.getDatatypename());
					definableAttrib.setAttributeinternalname(attrib.getInternalname());
					definableAttrib.setForvariant(false);
					definableAttrib.setMandatory(false);
					definableAttrib.setAleas("");

					this.dsAssignedAttribs.add(definableAttrib);
				}
			});
			this.updateToAssignDataSource();
		}
		else if (removedSelection != null && removedSelection.size() > 0 && isUserOriginated)
		{
			removedSelection.forEach(attrib -> {
				...
				{
					this.dsAssignedAttribs.removeIf(a -> a.getAttributeid().equals(attrib.getId()));
				}
			});
			this.updateToAssignDataSource();
		}
	...
	private boolean containSameAttrib(AttributeDTO attrib)
	{
		return this.dsAssignedAttribs.stream().filter(a -> a.getAttributeid().equals(attrib.getId())).findAny().isPresent();
	}


	private void updateToAssignDataSource()
	{
		ListDataProvider<DefinableAttributeDataDTO> dataprovider = new ListDataProvider<>(this.dsAssignedAttribs);
			...
			for (AttributeDTO attributeDTO : this.listFound)
			{
				if (this.dsAssignedAttribs != null && this.dsAssignedAttribs.size() > 0)
				{
					this.dsAssignedAttribs.forEach(f -> {
						if (attributeDTO.getId().equals(f.getAttributeid()))
						{
	...
	private void initilizeSelectedAttribsGrid()
	{
		...
		if (this.selectedDefAttribList != null && this.selectedDefAttribList.size() > 0){
			...
			ListDataProvider<DefinableAttributeDataDTO> dataprovider = new ListDataProvider<>(this.selectedDefAttribList);
			this.datGrid_SelectedAttribs.setDataProvider(dataprovider);
		}
	...
	{
		TemplateGenAttributesEditor senderReport = (TemplateGenAttributesEditor) sender;
	...
	private void btnUpdateAttribs_clickHandler(ClickEvent e)
	{
		if (this.dsAssignedAttribs != null && this.dsAssignedAttribs.size() > 0)
		{
			BbrEvent ev = new BbrEvent(BbrEvent.ITEMS_SELECTED);
			ev.setResultObject(this.dsAssignedAttribs);

			this.dispatchBbrEvent(ev);
			this.close();
		...
		else
		{
			...
			I18NManager.getI18NString(BbrUtilsResources.RES_MODULES_FEP, "no_attributes_selected"));
		}
