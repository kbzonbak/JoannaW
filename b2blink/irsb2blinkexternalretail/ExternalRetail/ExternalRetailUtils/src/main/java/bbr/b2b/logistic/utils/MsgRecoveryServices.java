...
import bbr.b2b.extractors.constants.ConnectorConstants;
...
public class MsgRecoveryServices {
	...
	private void getParams(String ruta) throws OperationFailedException {
		try {
			config = ResourceBundle.getBundle("bbr.b2b.logistic.utils.msgrec");
			// Tipo de archivo
			file_type = config.getString("bbr.msgrec.file.type");
			filecsv_type = config.getString("bbr.msgrec.filecsv.type");
	
			if ("windows".equalsIgnoreCase(ConnectorConstants.getInstance().getOS())) {
			...
			} else if ("linux".equalsIgnoreCase(ConnectorConstants.getInstance().getOS())) {
				// TESTING
				if (!ConnectorConstants.getInstance().getProduccion()) {
					// Carpetas donde se graba los mensajes
	...
	private void mailMe(String msg, String msgetype, String path, Exception exception) {
		...
		String subject = "";
		try {
			subject = ConnectorConstants.getInstance().getBUSINESSAREA() + ConnectorConstants.getInstance().getCOUNTRYCODE() +"Logistica: Mensaje no se quiere procesar. MsgeType :" + msgetype;
			Mailer mailer = Mailer.getInstance();
			String[] mailreciever = ConnectorConstants.getInstance().getDEVELOPER_MAIL_ERROR();
			// String mailsender = Constants.getPropertyValue("mailsender");
			String mailsender = ConnectorConstants.getInstance().getMailSender();
			String mailSession = ConnectorConstants.getInstance().getMAIL_SESSION();
	 ...
	 */
	public HashMap<String, String> readMsgsFromFiles(String messagetype) throws Exception {
	 ...
	 * del archivo depende del tipo de mensaje y la hora de creaci�n.
	 */
