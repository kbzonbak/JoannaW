...
import bbr.b2b.logistic.utils.ExternalLogisticStatusCodeUtils;
	...
	private RESTConnection(){
		...
		try{
			...
			if(isProxy){
				...
				Integer VeVProxyPort = Integer.valueOf(B2BSystemPropertiesUtil.getProperty("PROXY_PORT"));
	...
	public GenericResponseResultDTO executeRequest(HttpUriRequest request) {
		...
		try {
			...
			case HttpResponseCodes.SC_UNAUTHORIZED:
				result =  ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "401");
				break;

			case HttpResponseCodes.SC_FORBIDDEN:
				responseStr = convert(response.getEntity().getContent(), Charset.forName("UTF-8"));
				result =  ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "403");
				result.setJsonResponse(responseStr);
				break;
			case HttpResponseCodes.SC_NOT_FOUND:
				result =  ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "404");
				break;
			case HttpResponseCodes.SC_CONFLICT:
				result =  ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "409");
				break;	
			case HttpResponseCodes.SC_INTERNAL_SERVER_ERROR:
				responseStr = convert(response.getEntity().getContent(), Charset.forName("UTF-8"));
				result =  ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "500");
				result.setJsonResponse(responseStr);
				break;	
			...
			e.printStackTrace();
			return ExternalLogisticStatusCodeUtils.getInstance().setStatusCode(result, "500");
		}finally {
			try{
				if (response != null)
					response.close();
