...
public class MsgRecoveryServices {
	...
	private void getParams(String ruta) throws OperationFailedException {
		try {
			...
			if ("windows".equalsIgnoreCase(LogisticConstants.getInstance().getOS())) {
			...
			} else if ("linux".equalsIgnoreCase(LogisticConstants.getInstance().getOS())) {
				// TESTING
				if (!LogisticConstants.getInstance().getProduccion()) {
					// Carpetas donde se graba los mensajes
	...
	private void mailMe(String msg, String msgetype, String path, Exception exception) {
		...
		String subject = "";
		try {
			subject = LogisticConstants.getInstance().getBUSINESSAREA() + LogisticConstants.getInstance().getCOUNTRYCODE() +"Logistica: Mensaje no se quiere procesar. MsgeType :" + msgetype;
